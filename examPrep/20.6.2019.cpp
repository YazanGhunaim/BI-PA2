...
public:
    ...
    {
        for (const auto &[station, cnxns] : src.m_Connections)
        {
            cout << station << " : ";
...
void CTransport::AddLine(istream &is)
{
    ...
    {
        m_Connections[prev].insert(curr);
        prev = curr;
    }
    m_Connections[prev].insert(begin);
}
...
map<string, int> CTransport::FindDest(const string &from, int costMax) const
{
    ...
    result[from] = 0;
    if (costMax == 0 || m_Connections.find(from) == m_Connections.end())
        return result;
    ...
    while (!queue.empty())
    {
        ...
        for (const auto &stop : m_Connections.at(current))
        {
            if (result.find(stop) == result.end())
            {
                int cost = result[current] + 1;

                if (cost > costMax)
                    continue;

                queue.push(stop);
                result.insert({stop, result[current] + 1});
            }
        }
    }
    return result;
