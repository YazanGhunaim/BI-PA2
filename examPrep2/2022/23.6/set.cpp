...
CNetwork::CNetwork(const CNetwork &other)
    : m_Table(new CPerson *[other.m_Size] {}), m_Size(other.m_Size)
{
    for (size_t i = 0; i < m_Size; ++i)
    {
        ...
        while (head)
        {
            CPerson *newPerson = new CPerson(head->m_Name);
    ...
    for (size_t i = 0; i < m_Size; ++i)
    {
        ...
        while (head)
        {
            CPerson *person = Find(head->m_Name);

            for (CPerson *it : head->m_Friends)
            {
                person->m_Friends.push_back(Find(it->m_Name));
            }
